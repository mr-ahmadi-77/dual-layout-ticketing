    ...
    <RoleGate allow={["buyer"]}>
      <div className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-12">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Upcoming events, order history, and QR tickets ready to scan.</p>
        </div>
            ...
            <div key={event.id} className="flex flex-col overflow-hidden rounded-lg border border-border bg-card">
              ...
              <div className="flex flex-1 flex-col gap-2 p-4">
                ...
                <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
                  <span className="rounded-full bg-accent px-2.5 py-0.5 text-xs font-medium text-accent-foreground">Paid</span>
                  <Link to="/events/$id" params={{ id: event.id }} className="inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:opacity-80">
                    <QrCode className="size-4" /> View ticket
                  </Link>
                </div>
              </div>
            </div>
